...
angular.module('werpiApp')
  .controller('ProfileCtrl', function ($scope, $stateParams, Session, $rootScope, api, notify, $window) {
    ...
    $scope.newUserData.phoneTypeId='1';
    ...
    $scope.setNewUserData = function (newUserData) {
      if ($scope.profile_form.$valid) {
        ...
        if (!$scope.newUserData.provinceId){
            notify({ messageTemplate: '<i class="fa fa-exclamation-circle big-fa"></i><div class="notify-text"> Seleccione una provincia del listado</div>', classes: 'alert-notify', position: 'right' });
            return;
        }
